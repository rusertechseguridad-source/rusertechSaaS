      ...
      <div className="bg-[#1A2346] border border-[#2D3B6A] rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center p-5 border-b border-[#2D3B6A]">
          <div className="flex items-center gap-3 text-red-400">
            ...
            <h2 className="text-lg font-semibold tracking-wide">{t('securityKeys.deleteModal.title')}</h2>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="p-5 text-gray-300 space-y-4">
          <p>
            {t('securityKeys.deleteModal.message').replace('{{name}}', securityKey.name)}
          </p>
          <div className="p-3 bg-orange-500/10 border border-orange-500/20 rounded-lg text-orange-400 text-sm">
            {t('securityKeys.deleteModal.warning')}
          </div>
        </div>
        ...
        <div className="p-5 border-t border-[#2D3B6A] flex justify-end gap-3 bg-[#151B36]">
          ...
          >
            {t('securityKeys.deleteModal.cancel')}
          </button>
          <button
            onClick={onConfirm}
            disabled={isDeleting}
          ...
          >
            {isDeleting ? (
              ...
              t('securityKeys.deleteModal.confirm')
            )}
          </button>
        </div>
      </div>
